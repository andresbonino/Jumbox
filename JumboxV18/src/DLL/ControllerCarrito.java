...
public class ControllerCarrito<T extends Carrito> implements CarritoRepository {
	...
	public void compras(LinkedList<Productos> productos, Cliente cliente) {
		...
		try {
			...
			// ELEGIR SUCURSAL
			OpcionesSucursales opcionSeleccionada = (OpcionesSucursales) JOptionPane.showInputDialog(null,
					"¿En qué sucursal querés comprar?", "Jumbox", JOptionPane.QUESTION_MESSAGE, null,
					OpcionesSucursales.values(), OpcionesSucursales.values()[0]);
			...
			PreparedStatement stmt = Conexion.getInstance().getConnection()
					.prepareStatement("SELECT a.fk_producto, a.cantidad, p.nombre, p.precio, p.id_producto "
							+ "FROM almacen_sucursal a " + "JOIN producto p ON a.fk_producto = p.id_producto "
			...
			while (rs.next()) {
				...
				sb.append("- ").append(nombre).append(" | Precio: $").append(precio).append(" | Stock: ").append(stock)
						.append("\n");
			}
			...
			String seleccion = (String) JOptionPane.showInputDialog(null, sb.toString(), "Agregar al carrito",
			...
			if (productoElegido != null) {
				...
				if (this.sucursalSeleccionada == null) {
					...
					JOptionPane.showMessageDialog(null,
							"Ya estás comprando en la sucursal: " + this.sucursalSeleccionada.getId_Sucursal()
				...
				do {
					...
					try {
						...
						if (cantidad <= 0) {
							...
							PreparedStatement stmtProductoId = con
									.prepareStatement("SELECT id_producto FROM producto WHERE nombre = ?");
	...
	public void verCarrito(Cliente cliente, Sucursal sucursal) {
		...
		for (Carrito item : carrito) {
			...
			if (item.getCantidad() > 10) {
				...
				resumen.append(item.getProducto().getNombre()).append(" x ").append(item.getCantidad()).append("(Con descuento)= $")
						.append(item.getTotal()*0.9).append("\n");
		...
		int seleccion = JOptionPane.showOptionDialog(null, resumen.toString() + "\n\n¿Desea realizar la compra?",
				"Carrito", JOptionPane.DEFAULT_OPTION, JOptionPane.QUESTION_MESSAGE, null, opciones, opciones[0]);
	...
	public LinkedList<Carrito> obtenerProductosCarrito(int idCarritoActual) {
		...
		try {
			PreparedStatement stmt = con
					.prepareStatement("SELECT p.nombre, p.precio, pc.cantidad " + "FROM producto_carrito pc "
							+ "JOIN producto p ON pc.fk_producto = p.id_producto " + "WHERE pc.fk_carrito = ?");
			stmt.setInt(1, idCarritoActual);
			ResultSet rs = stmt.executeQuery();

			while (rs.next()) {
				Productos producto = new Productos(rs.getString("nombre"), rs.getDouble("precio"),
						rs.getInt("cantidad"));
	...
	public void crearNuevoCarrito(Cliente cliente) {
		try (Connection con = Conexion.getInstance().getConnection()) {
			PreparedStatement stmt = con.prepareStatement("INSERT INTO carrito(fk_cliente) VALUES (?)",
					Statement.RETURN_GENERATED_KEYS);
	...
	public LinkedList<Carrito> obtenerCarrito(int idCarrito) {
		...
		try {
			PreparedStatement stmt = con.prepareStatement(
					"SELECT pc.fk_producto, pc.cantidad, p.nombre, p.precio " + "FROM producto_carrito pc "
							+ "JOIN producto p ON pc.fk_producto = p.id_producto " + "WHERE pc.fk_carrito = ?");
	...
	public void realizarCompra(LinkedList<Carrito> carrito, Cliente cliente, Sucursal sucursal, int idCarritoActual) {
		try {
			...
			PreparedStatement psBorrar = con.prepareStatement("DELETE FROM producto_carrito WHERE fk_carrito = ?");
			...
			PreparedStatement psBorrarCarrito = con.prepareStatement("DELETE FROM carrito WHERE id_carrito = ?");
